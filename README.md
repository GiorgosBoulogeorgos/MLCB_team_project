...
│   ├── MLCB_report.tex         # report source (single source of truth)
│   ├── MLCB_report.pdf         # report, PDF
│   ├── MLCB_report.docx        # report, Word — generated by make_docx.py
│   ├── make_docx.py            # LaTeX → .docx build (pandoc); see below
│   ├── Boulogeorgos_Mici.pptx  # presentation
│   ├── Boulogeorgos_Mici.pdf   # presentation, PDF
│   ├── figures/                # figures used by the report
│   └── paper/                  # Maitra et al. (2023), the original paper
...
`report/MLCB_report.tex` is the single source of truth. The PDF and the Word
version are generated from it:

```bash
cd report
pdflatex MLCB_report.tex && pdflatex MLCB_report.tex   # -> MLCB_report.pdf
python3 make_docx.py                                   # -> MLCB_report.docx (needs pandoc)
```

`make_docx.py` rewrites the LaTeX into a pandoc-friendly form first, because Word
cannot render TikZ, `subfigure`s or LaTeX cross-references: the pipeline diagram is
swapped for the pre-rendered `figures/pipeline_diagram.png`, subfigures are split
into one figure per panel, and `\ref`/`\cite` are resolved to literal numbers read
from `MLCB_report.aux` — so the `.docx` and the `.pdf` always agree. Run `pdflatex`
before `make_docx.py` so the `.aux` is current.

## AI usage

Generative AI (Anthropic's Claude) was used as a coding/writing assistant: initial
pipeline scaffold, code debugging (notably Phase B / LIANA), figure-plotting code,
and code review. It was not used to generate data or decide the scientific
conclusions. See the **AI usage disclosure** section of the report for the full
statement.

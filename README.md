...
│   ├── MLCB_report.docx        # report, Word (same source)
...
`report/MLCB_report.tex` is the single source of truth:
...
`report/MLCB_report.docx` is the same report in Word form, generated from the
same `.tex`.

...
\author{MLCB Team Project \\ \small Re-analysis of Maitra \textit{et al.}
...
The analysis proceeds in four phases (Figure~\ref{fig:pipeline}): reproduction
(A), cell--cell communication inference (B), machine learning (C), and an
interpretability layer (Step~4). Each expensive step is checkpointed so a
runtime timeout never costs more than one step.
...
\node[box=phaseA, right=of geo] (a1)
  {\textbf{A. Reproduction}\\[1pt]\scriptsize QC, CP1e4+log1p,\\merge M24,
   attach labels};
...
% ---- Phase C ----
\node[box=phaseC, below=12mm of b1] (c1)
...
\node[data, below=7mm of b3, yshift=-1mm] (bf)
...
\code{log1p}, preserving raw counts in \code{.layers['counts']} (pseudobulk in
...
% ===========================================================================
\section{Phase B — Cell--cell communication}
% ===========================================================================
...
dropped $\rightarrow$ 156{,}911 nuclei). With \code{min\_cells=10} per donor,
...
with elbow-selected \textbf{rank~5} yields five \emph{communication programs}; a
...
% ===========================================================================
\section{Phase C — Machine learning}
\label{sec:phaseC}
% ===========================================================================
...
20 Optuna trials per inner search, evaluating five estimators---logistic
regression (LR), Gaussian naive Bayes (GNB), linear discriminant analysis (LDA),
random forest (RF), and XGBoost~\cite{xgboost}. Bootstrap 95\% confidence
...
canonical Factor~1\ldots5 by $|{\rm corr}|$ Hungarian assignment.
\end{itemize}
...
\subsection{Step 4 — interpretation and the directed hypothesis test}

We probe \factorfour\ from three angles with SHAP (signed log-odds
...
\begin{subfigure}{0.49\textwidth}
  ...
  \caption{In-fold leakage-safe factors on held-out donors: \factorfour\
  becomes the \#1 communication factor.}
...
Code lives in git (\code{src/}, \code{notebooks/}); data and checkpoints live on
Google Drive (gitignored). Key modules: \code{src/functions.py} (Phase A/B),
\code{src/tensor\_features.py} (in-fold projector), \code{src/rncv.py} (repeated
nested CV), \code{src/feature\_selection.py} (\code{VarianceTopK},
\code{MRMRSelector}), \code{src/estimators.py}, \code{src/shap\_step4.py}
(Step~4), and the local runner \code{run\_rigorous\_local.py}. Rigorous run:
seed~42, 3$\times$5$\times$3 nested CV, 20 Optuna trials, per-fold tensor refit;
LIANA~1.7.3 / cell2cell~0.8.4 for Phase~B.
